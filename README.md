...
Because certain particle physics events can be somewhat rare, the DECO dataset only contains a few thousand images. In order to prevent the model from simply memorizing the training set, random rotations, x-y reflections, and blurring are used during the training process. This also should force the model to learn features that are approximately rotation-invariant. Below is an example of what the input data looks like (before augmentation):


During training, samples images were periodically produced by the generative model:
## Snapshot 1
...
The generated images aren't perfect, but the model certainly appears to have learned the correct features. 
